...
def CountOccurencesInText(word, text):
    ...
    pattern = (
        (re.match("[a-z]", word, re.I) != None) * "(?<![a-z])((?<!')|(?<=''))"
        + word
        + (re.match("[a-z]", word[-1], re.I) != None) * "(?![a-z])((?!')|(?=''))"
    )
...
and my name is GEORGES"""
    ...
    assert 0 == CountOccurencesInText("n", text)
    assert 1 == CountOccurencesInText("true", text)
    assert 0 == CountOccurencesInText(
        "reflexion mirror",
    ...
    assert 1 == CountOccurencesInText("get back to me", u"When you know:get back to me")
    assert 1 == CountOccurencesInText(
        "don't be left",
...
Don't be left unprotected. Order your don SSSS3000 today!""",
    ...
    assert 1 == CountOccurencesInText("don't", "I don't take that as a 'yes'?")
    assert 1 == CountOccurencesInText(
